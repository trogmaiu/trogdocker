...
##############################################################################
## Bloco que tem repetição para criar os formularios, saí no arquivo images2.php
iii=0
...
##############################################################################
## Bloco que fecha colocando o </body> </html>
...
while [ $i -le $tt ]
do
cut -c 43-55 /var/www/trogmaiu/public_html/images-resultado1.txt | sed -n "${i}s/^/\$b${i}\ = \"/p" >> /var/www/trogmaiu/public_html/temp/images-resultado2.txt
   ...
   if(isset(\$_GET[\"vai$ii\"]))
     {
      ...
      fwrite(\$fp, \"/usr/bin/xterm -e /usr/bin/docker run -ti \$b$ii\");
      fclose(\$fp);       
}
